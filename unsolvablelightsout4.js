...
Vue.component('lightsout', {
	...
	for (let i = 0; i < this.n; i++){
		...
		if (i==0 && j==this.n-1){
		    statarr.push({x:i,y:j,v:0,idx:i*this.n+j});
		}
		else{
    ...
    template: '<div><p>{{ msg }}</p><svg width="450" height="400"> <cell v-for="(s,id) in status" v-on:toggle="pushed($event)" v-bind:key=id :idx=s.idx v-bind:x=s.x v-bind:y=s.y v-bind:v=s.v></cell></svg></div>',
    methods:{
	pushed: function(key){
...
lightsout = new Vue({
    el: '#lightsout4',
    template: '<lightsout n="4"></lightsout>'
})
