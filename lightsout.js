...
Vue.component('lightsout', {
    ...
    template: '<div><p>{{ msg }}</p><svg width="350" height="300"> <cell v-for="(s,id) in status" v-on:toggle="pushed($event)" v-bind:key=id :idx=s.idx v-bind:x=s.x v-bind:y=s.y v-bind:v=s.v></cell></svg><button v-on:click="shuffle()">Shuffle</button></div>',
    methods:{
	pushed: function(key){
